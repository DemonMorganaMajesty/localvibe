...
| 编号 | 需求描述 | 指标 |
|------|----------|------|
| NFR-01 | 秒杀接口响应时间（不含异步订单处理） | ≤ 200ms（P95） |
| NFR-02 | 系统吞吐量（秒杀场景） | 支持 5000 QPS 以上（单机） |
| NFR-03 | 缓存命中率（热点数据） | ≥ 95% |
| NFR-04 | 秒杀超卖率 | 0（绝对禁止） |
| NFR-05 | 缓存不一致容忍时间 | ≤ 5 秒（最终一致性） |
| NFR-06 | MQ 消息堆积容忍 | 可承受百万级消息堆积，不影响消费者稳定性 |
| NFR-07 | 系统可用性 | ≥ 99.9%（排除外部依赖故障） |
| NFR-08 | 冷启动后首次请求响应时间 | ≤ 500ms（预热后） |
...
你不要写测试数据和指标进去, 我要拿去面试的,我怕被拷打，
好的，用户这次提出了一个非常具体的要求：不要写测试数据和指标进去，因为这是要拿去面试的，怕被拷打。

嗯，回顾之前的对话，用户一直在让我帮忙写技术需求文档，先是基于一个多Agent旅行规划系统，后来是一个社区电商平台。我之前两次都提供了非常详细、结构化的MD格式文档，里面包含了大量的性能指标（比如响应时间≤200ms、QPS支持5000等）和测试要求。

用户现在的深层需求很明显：他/她需要一份文档去面试时展示，但担心面试官会追问文档里写的那些具体数值（比如“你怎么测出QPS 5000的？”“响应时间200ms是怎么优化得到的？”），如果自己没有实际测试过或者数据不扎实，很容易被问住，反而弄巧成拙。

所以，用户要的是一份更“概念化”、更偏向于架构设计和技术思路阐述的文档，而不是一份带有具体量化指标的工程实施方案。目的是展示对技术栈和架构的理解，而非证明系统达到过某个具体性能水平。

我需要调整之前那种包含详细指标、测试用例、版本规划的写法。应该把重点放在“如何设计”、“为什么这么设计”、“解决了什么问题”上。把“指标”部分换成“目标”或“要求”，用更定性的语言描述，比如“高吞吐”、“低延迟”、“绝对禁止超卖”，而不给出具体数字。测试部分也可以简化，只谈策略和工具，不谈具体达标值。

这样用户拿到文档后，可以胸有成竹地介绍架构思路，而不会被一个具体的数字或测试细节难住。文档的整体结构可以保留，但内容需要“软化”掉那些可量化的部分。

好，思路清晰了，准备开始重写一份面试友好的版本。
...
| 编号 | 需求描述 | 要求 |
|------|----------|------|
| NFR-01 | 秒杀接口响应时间 | 快速响应，用户体验无延迟感 |
| NFR-02 | 系统吞吐量 | 支持高并发场景，具备弹性扩展能力 |
| NFR-03 | 缓存命中率 | 热点数据应尽量从缓存获取，减少数据库访问 |
| NFR-04 | 秒杀超卖率 | 绝对禁止超卖 |
| NFR-05 | 缓存不一致容忍时间 | 保证最终一致性，不一致窗口尽可能短 |
| NFR-06 | MQ 消息堆积容忍 | 可承受大规模消息堆积，不影响消费者稳定性 |
| NFR-07 | 系统可用性 | 核心链路具备容错与降级能力 |
| NFR-08 | 冷启动后首次请求响应时间 | 预热后响应流畅，无明显延迟 |
...
| 层级 | 技术 | 选型理由 |
|------|------|----------|
| 后端框架 | Spring Boot 3.x + Spring MVC | 成熟稳定，支持异步处理与 AOP |
| ORM | MyBatis-Plus | 简化 CRUD，支持分页、逻辑删除 |
| 数据库 | MySQL 8.0 | 支持事务、行锁、binlog 格式 ROW |
| 缓存（远程） | Redis 哨兵模式 | 高可用，自动故障转移，支持 Lua 脚本、GEO、Bitmap |
| 缓存（本地） | Caffeine | 高性能 JVM 缓存，支持 TTL、最大容量 |
| 网关缓存 | OpenResty | 基于 Nginx + Lua，可实现高级缓存策略 |
| 消息队列 | RocketMQ | 高吞吐、低延迟，支持事务消息、死信队列 |
| 数据同步 | Canal | 监听 MySQL binlog，实现缓存失效、数据同步 |
| 分布式锁（备选） | Redisson | 提供可重入锁，用于非秒杀场景的并发控制 |
| 序列化 | Jackson | 高效 JSON 序列化 |
